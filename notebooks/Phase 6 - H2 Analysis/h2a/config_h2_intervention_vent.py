"""
Minimal configuration for H2a Analysis (Mechanical Ventilation).
Based on champion model from Figure 5: F3_P0 (text-embedding-004).
"""
import os
...
class ConfigH2:
    def __init__(self):
        ...
        self.BASELINE_MODEL_PATH = os.path.join(num_dir, 'intervention_vent', 'model_1_xgboost_baseline_calibrated.pkl')

        # Champion embedding model and data (text-embedding-004)
        self.CHAMPION_ARM = 'F3_P0'
        self.CHAMPION_MODEL_PATH = os.path.join(self.NOTEBOOKS_DIR, 'Phase 5', 'embedding_model_results', 'text-embedding-004', 'intervention_vent', f'model_{self.CHAMPION_ARM}_calibrated.pkl')
        ...
        # Outputs
        self.OUTPUT_DIR = os.path.join(current_dir, 'h2_results_intervention_vent')
        os.makedirs(self.OUTPUT_DIR, exist_ok=True)

        # Core settings required by runner
        self.TARGET_VARIABLE = 'intervention_vent'

...
# Determine project root assuming the script is in a subdirectory of the project
PROJECT_ROOT = os.path.abspath(os.path.join(os.path.dirname(__file__), '..', '..'))

# Configuration
DEFAULT_CONFIG = {
    'HDF_FILE_PATH': os.path.join(PROJECT_ROOT, 'data', 'raw', 'all_hourly_data.h5'),
    'FEATURE_CLASSIFICATION_PATH': os.path.join(PROJECT_ROOT, 'data', 'processed', 'eda_results_corrected', 'feature_classification.csv'),
    'OUTPUT_DIR': 'notebooks\Phase 1 and 2\phase_1_outputs',
    'DRY_RUN': False,
...
def setup_logging():
    ...
    logging.basicConfig(
        ...
        handlers=[
            logging.FileHandler(os.path.join(CONFIG['OUTPUT_DIR'], 'preprocessing_log.txt'), mode='w', encoding='utf-8'),
...
def load_data():
    ...
    with pd.HDFStore(CONFIG['HDF_FILE_PATH'], 'r') as store:
        ...
        df_interventions = store['/interventions'] if '/interventions' in store else pd.DataFrame()
        
        if 'los_icu' in df_patients.columns:
            df_patients['los_3'] = (df_patients['los_icu'] > 3).astype(int)
            df_patients['los_7'] = (df_patients['los_icu'] > 7).astype(int)
            logging.info("Created LOS targets: los_3, los_7")
        else:
            logging.error("'los_icu' column not found in patients data. Cannot create LOS targets.")
            # The script will likely fail later if these columns are not found in TARGET_VARIABLES
        ...
        if CONFIG['DRY_RUN']:
            ...
            if not df_interventions.empty:
                df_interventions = df_interventions[df_interventions.index.get_level_values('icustay_id').isin(sampled_ids)]
    ...
    if 'age' in df_patients.columns:
        ...
        if not df_interventions.empty:
            df_interventions = df_interventions[df_interventions.index.get_level_values('icustay_id').isin(df_patients.index.get_level_values('icustay_id'))]
        
        logging.info(f"Age filtered: {len(valid_subjects)} patients, {df_ts.shape[0]} records")
    
    return df_patients, df_ts, df_interventions

def _encode_and_store(series, col_name):
    """Fills NA, encodes a series, and returns the encoded series and the encoder info."""
    logging.info(f"Encoding column: {col_name}")
    # Fill missing values with the mode
    mode_val = series.mode().iloc[0] if not series.mode().empty else 'Unknown'
    series_filled = series.fillna(mode_val)
    
    # Encode
    le = LabelEncoder()
    encoded_series = le.fit_transform(series_filled.astype(str))
    
    # Create encoder info for storage
    encoder_info = {
        'encoder': le,
        'classes': le.classes_.tolist(),
        'mapping': {i: cls for i, cls in enumerate(le.classes_)}
    }
    logging.info(f"'{col_name}' encoded. Encoder can be stored for reversal.")
    return pd.Series(encoded_series, index=series.index, name=col_name), encoder_info
...
def process_demographics(df_patients):
    ...
    cols_to_select = [col for col in demographic_cols if col in df_patients.columns]
    df_demo = df_patients.groupby(level='subject_id').first()[cols_to_select].copy()

    # Rebuild the DataFrame to ensure only processed columns are kept
    df_demo_processed = pd.DataFrame(index=df_demo.index)
    
    # Handle age (numeric)
    if 'age' in df_demo.columns:
        df_demo_processed['age'] = df_demo['age'].fillna(df_demo['age'].median())
    ...
    for col in categorical_cols:
        if col in df_demo.columns:
            # Assume these are categorical and encode them directly
            encoded_series, encoder_info = _encode_and_store(df_demo[col], col)
            df_demo_processed[f'{col}_encoded'] = encoded_series
            label_encoders[f'{col}_encoded'] = encoder_info
    
    logging.info(f"Demographics processed: {df_demo_processed.shape}")
    logging.info(f"Demographic encoders: {list(label_encoders.keys())}")
    return df_demo_processed, label_encoders
...
def verify_no_categorical_features(datasets):
    ...
    if non_numeric_cols:
        logging.warning(f"Unexpected non-numeric features found: {non_numeric_cols}")
        return False
    else:
...
def process_targets(df_patients, df_interventions, label_encoders):
    """Create intervention targets and encode categorical targets."""
    df_patients_processed = df_patients.copy()

    # Create intervention targets, excluding prevalent cases
    if not df_interventions.empty:
        logging.info("Creating intervention targets and handling prevalent cases...")
        
        # Define intervention window boundaries
        df_interventions_future = df_interventions[df_interventions.index.get_level_values('hours_in') >= CONFIG['WINDOW_SIZE']]
        df_interventions_prevalent = df_interventions[df_interventions.index.get_level_values('hours_in') < CONFIG['WINDOW_SIZE']]

        for intervention_col in ['vent', 'vaso']:
            target_col_name = f'intervention_{intervention_col}'
            
            if intervention_col in df_interventions.columns:
                # 1. Identify future use of the intervention (the outcome)
                future_target = df_interventions_future.groupby('icustay_id')[intervention_col].max().reindex(
                    df_patients_processed.index.get_level_values('icustay_id').unique()
                ).fillna(0).astype(int)
                df_patients_processed[target_col_name] = df_patients_processed.index.get_level_values('icustay_id').map(future_target).fillna(0)
                logging.info(f"Created '{target_col_name}' target.")

                # 2. Identify prevalent cases (patients already on the intervention in the first 24h)
                prevalent_ids = df_interventions_prevalent[df_interventions_prevalent[intervention_col] > 0].index.get_level_values('icustay_id').unique()
                
                # 3. Exclude prevalent cases by setting their target to NaN
                if len(prevalent_ids) > 0:
                    prevalent_mask = df_patients_processed.index.get_level_values('icustay_id').isin(prevalent_ids)
                    df_patients_processed.loc[prevalent_mask, target_col_name] = np.nan
                    logging.info(f"Flagged {prevalent_mask.sum()} prevalent cases for '{target_col_name}' by setting target to NaN.")
    
    return df_patients_processed, label_encoders
...
def create_splits(df_patients):
    ...
    subject_outcomes = df_patients.groupby('subject_id')[CONFIG['STRATIFICATION_TARGET']].max()
...
def main(config_dict=None):
    ...
    df_patients, df_ts_raw, df_interventions = load_data()
    ...
    # Create and encode targets
    df_patients, label_encoders = process_targets(df_patients, df_interventions, label_encoders)
    ...
    logging.info("Target variable means:")
    for split in ['train', 'val', 'test']:
        logging.info(f"  {split}:\n{datasets[f'y_{split}'].mean().to_string()}")
